...
### 大問1. バーチャルホストの無効化・再有効化

ハンズオンで作った `shop.hanamall.local` の設定ファイルを一度 `a2dissite` で無効化し、再度 `a2ensite` で有効化する手順を完全に自力でやってみる

### 大問2. エラーログを使ったデバッグ

`shop.hanamall.local` の DocumentRoot を存在しないパスに変えて 403/404 を出し、`error.log` だけを手がかりに直す（answers/ は見ない）

### 大問3. 応用：新規バーチャルホスト追加

新たに `api.hanamall.local` というバーチャルホストを自力で追加し、`{"status":"ok"}` というテキストを返すようにする

### 大問4. 思考問題

本番サービス中に `sudo systemctl reload apache2` を実行したとき、もし設定ファイルに文法エラーがあるとどうなるか？（サービスは落ちるか？）実際に試して答えよ
