...
`dig` で以下を調べて結果を報告せよ
   - `google.com` の A レコードと TTL
   - `google.com` の MX レコード（メールサーバー）
   - TTL が短いドメインと長いドメインで何が違うか説明せよ
...
firewalld で以下の操作を行い、各ステップの結果を記録せよ
   - `8080/tcp` を開放 → `curl http://localhost:8080` で確認 → 閉じる
   - `--permanent` を付け忘れた場合どうなるか、実際に試して確認せよ

### 大問5. 以下のネットワーク調査コマンドを実行し、それぞれ何を調べるコマンドかを説明せよ

以下のネットワーク調査コマンドを実行し、それぞれ何を調べるコマンドかを説明せよ
   ```bash
   traceroute 8.8.8.8
   ss -s
   cat /etc/resolv.conf
...
**思考問題:** `ss -tnlp` に見慣れないポート（例: `0.0.0.0:4444`）が表示されていたらどう対処するか？手順を3ステップで答えよ