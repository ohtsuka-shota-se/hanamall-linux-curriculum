...
`ip a` と `ip r` の出力を見て「このサーバーのIPとゲートウェイ」を答えられるか確認する

### 大問2. DNSとTTLの理解

`dig` で `google.com` を引き、TTLの値と「その意味（なぜこの秒数が設定されているのか）」を自分の言葉で説明する

### 大問3. リスニングポートの確認

`ss -tnlp` の出力から「22番と80番が何のサービスか・なぜLISTENしているのか」を説明する

### 大問4. 応用：ファイアウォール操作

firewalld で 8080/tcp を開放 → `ss -tnlp` で確認 → 閉じる、の一連をコマンド履歴なしで手順を暗記してやってみる

### 大問5. 思考問題

`ss -tnlp` に見慣れないポートが表示されていたらどう対処するか？手順を3ステップで答えよ
