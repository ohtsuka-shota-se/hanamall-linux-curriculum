...
`du` コマンドでディスク使用量トップ10のディレクトリを特定し、「なぜそこが大きいか」を1行で説明する

### 大問2. ddとdfの挙動確認

`dd` で 500MB のテストファイルを作り、`df` で変化を確認後に削除する。「削除してもdfの数値が変わらない」場合の原因と対処を調べよ（ヒント: lsof）

### 大問3. 仮想ディスクの追加とマウント永続化

仮想ディスクを追加して `/mnt/data` にマウントし、`/etc/fstab` に永続化する。再起動後にもマウントされていることを確認する

### 大問4. rsync による同期

`/var/www` を `/backup/www` に `rsync` で同期する。`--delete` オプションの意味と、**本番環境で使うときに注意すべき点**を答えよ

### 大問5. 思考問題

Week07 のバックアップスクリプトで `/backup/` がいっぱいになったら何が起きるか？どう対策するか？
